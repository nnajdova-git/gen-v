...
def client_fixture():
  with testclient.TestClient(veo.router) as test_client:
    yield test_client


@pytest.fixture(name='patch_env_settings', autouse=True)
def patch_env_settings_fixture(monkeypatch):
  monkeypatch.setattr(veo, 'env_settings', settings.EnvSettings())


@pytest.fixture(name='mock_generate_video')
def mock_generate_video_fixture():
  with mock.patch(
      'routers.veo.vertexai_component.generate_video', autospec=True
  ) as mock_generate:
...
def mock_fetch_operation_status_fixture():
  with mock.patch(
      'routers.veo.vertexai_component.fetch_operation_status', autospec=True
  ) as mock_fetch:
...
def mock_parse_gcs_uri_fixture():
  with mock.patch('components.gcs_storage.parse_gcs_uri') as mock_parse:
...
def test_veo_generate_video_returns_correct_response(
  ...
  response = client.post(
      '/generate', json=request.model_dump(exclude_unset=True)
  )
...
def test_veo_generate_video_returns_mock_response_with_use_mocks_true(
  ...
  veo.env_settings.use_mocks = True
...
def test_veo_operation_status_returns_correct_response(
  ...
  response = client.post(
      '/operation/status', json=request.model_dump(exclude_unset=True)
  )
  assert response.status_code == 200
  assert response.json() == {
      'name': 'projects/PROJECT_ID/operations/OPERATION_ID',
